...
import type { IAudioTrack } from "../../entities";
import useIsTrackEnabled from "../../hooks/useIsTrackEnabled";

let clipId = 0;
const getUniqueClipId = () => {
  clipId += 1;
  return clipId;
};
...
export function AudioLevelIndicator(props: AudioLevelIndicatorProps) {
  const color = "green";
  const { audioTrack } = props;
  const isTrackEnabled = useIsTrackEnabled(props.audioTrack);
  const SVGRectRef = useRef<SVGRectElement>(null);
  const [analyser, setAnalyser] = useState<AnalyserNode>();
  const mediaStreamTrack = audioTrack?.mediaStreamTrack ?? null;
